...
A generator for making request to OpenAlex API Returns one record at a
time.
...
- ...:
  ...
  arguments passed to the generator including \`query_url\`,
  \`api_key\`, and \`verbose\`. See \`oa_request\` for details on these
  arguments.
...
For the case without group_by, setting \`verbose = TRUE\` when you call
\`oar()\` for the first time will give the total number of records in
the message. With group_by, unfortunately we don't have a way to know
the total until we query until exhausted.
